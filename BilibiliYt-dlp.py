...
# 再询问我是否需要引入 cookies 文件？（输入“y”则为引入 cookies 文件，默认地址为"d:\\ProApps\\Youtube-dl\\BilibiliYt-dlpCookies.txt"；如果输入的是本地链接，则将该地址作为 cookies 文件地址；按回车或输入“N”则不引入 cookies 文件。）
...
def main():
    ...
    # 询问Cookies
    cookies_answer = input("是否使用Cookies文件？(Y/n, 默认“d:\\ProApps\\Youtube-dl\\BilibiliYt-dlpCookies.txt”): ").strip().lower()
    if cookies_answer == "n":
        cookies = None
    else:
