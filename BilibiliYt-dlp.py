...
# 首先询问我想下载单个链接还是列表文件中的链接？（如果输入的是网络链接，则下载该链接；按回车则为默认地址“e:\\Documents\\Creations\\Scripts\\Python\\BilibiliYt-dlpLists.txt”；如果输入的是本地链接，则下载该地址列表文件中的链接。）
# 再询问我是否需要引入 cookies 文件？（输入“y”则为引入 cookies 文件，默认地址为"e:\\Documents\\Creations\\Scripts\\Python\\BilibiliYt-dlpCookies.txt"；如果输入的是本地链接，则将该地址作为 cookies 文件地址；按回车或输入“N”则不引入 cookies 文件。）
...
DEFAULT_COOKIES_FILE = r"e:\\Documents\\Creations\\Scripts\\Python\\BilibiliYt-dlpCookies.txt"
DEFAULT_PROXY = "127.0.0.1:10809"
DEFAULT_SAVE_PATH = r"d:\\Downloads\\"

def main():
    # 询问下载方式
    input_link = input("请输入视频链接或列表文件路径（直接回车使用默认列表文件“e:\\Documents\\Creations\\Scripts\\Python\\BilibiliYt-dlpLists.txt”）：").strip()
    if not input_link:
        # 使用默认列表文件
    ...
    # 询问Cookies
    cookies_answer = input("是否使用Cookies文件？(Y/n, 默认“e:\\Documents\\Creations\\Scripts\\Python\\BilibiliYt-dlpCookies.txt”): ").strip().lower()
    if cookies_answer == "n":
        cookies = None
    else:
