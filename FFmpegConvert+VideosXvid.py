...
def compress_and_remux_video(source_path, target_folder, relative_path):
    ...
    temp_output_path = os.path.join(target_subfolder, f"{file_name}_temp.avi")  # Xvid 输出 AVI
    final_output_path = os.path.join(target_subfolder, f"{file_name}.avi")
    
    # 构建 ffmpeg 命令（Xvid 高质量压缩，音频 MP3 高质量）
    ffmpeg_command = [
        "ffmpeg", "-i", source_path,
        "-c:v", "mpeg4", "-vtag", "xvid", "-qscale:v", "1",
        "-c:a", "copy",
        "-c:s", "copy",
        "-map", "0",
        temp_output_path
    ...
    # 构建 mkvmerge 命令（Xvid 输出是 AVI，一般不再用 mkvmerge，可直接重命名或跳过）
    # 这里为了保留逻辑仍然用 mkvmerge 生成同名文件
    mkvmerge_command = [
