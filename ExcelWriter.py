...
# 在脚本开始前询问我 excel 文件位置（默认为：e:\Documents\Creations\Scripts\Attachment\标准.xlsx）、源文件夹位置（默认为：d:\Works\Downloads\），写入文件夹位置（默认为：d:\Works\In\），上传文件夹位置（默认为：d:\Works\Uploads\）。
# 询问我想要做什么：
...
# 4. 多文件复杂写入：读取 excel 文件，第一行为表头（字段名）。此后每一行为一条记录。分别询问我"引用页"、"属于"、"主链接"的值（按回车则为空）。遍历源文件夹内所有文件及子文件夹中的文件，顺序完成。1. 每一条记录新开一行。2. "Index"字段值为上一行 "Index"字段值+1（如上一行为空或为表头，则"Index"字段值为 1）。3. 将源文件夹名写入"名字"。将源文件夹内每个文件名写入临时文件“d:\Works\0\SourceName.txt”中（内容先清空，每行一个），最合合并写入"原文件名"字段值。4. 将该文件移动到写入文件夹下的子文件夹（子文件夹名字为源文件夹名，保持原来文件夹结构），并移动到"z:\"中，如果无法完成写入"z:\"中，则重命名原文件（从后删除1个字符（不包括扩展名）），再次尝试将原文件向目标文件夹移动。反复循环，直至能将原文件向目标文件夹移动。如果有重命名过文件，将修改过的文件名写入临时文件“d:\Works\0\ChangeName.txt”中（内容先清空，每行一个），最合合并写入"矫正文件名"字段值中。写入"z:\"后，系统会自动生成一个对应文件（自动加密的）。读取"d:\Xyz\"新生成的文件名，将其文件名（无扩展名）写入临时文件“d:\Works\0\EncryptedName.txt”中（内容先清空，每行一个），最合合并写入"加密文件名"字段值中。将"d:\Xyz\"新生成的文件移动到上传文件夹下的子文件夹（子文件夹名字为源文件夹名，保持原来文件夹结构，在"d:\Xyz\"中不保留）。5. 将原来询问我的"引用页"、"属于"、"主链接"的值写入"引用页"、"属于"、"主链接"字段值。6. 计算并生成该文件的 Ed2K 链接。生成的 ed2k 链接，写入临时文件“d:\Works\0\Ed2kList.txt”中（内容先清空，每行一个），最合合并写入"标准链接"字段值。7. 通过每个"标准链接"字段值，分别将分解出的"大小"、"散列"值，大小请转成 B、KB、MB、GB 形式，并精确到小数点后 4 位，hash 转全部大写。写入临时文件“d:\Works\0\Ed2kList.size.txt”、“d:\Works\0\Ed2kList.hash.txt”中（内容先清空，每行一个），最合合并写入"大小"、"散列"字段值。
# 0. 退出程序。
# 完成后，反复循环我要做什么。
