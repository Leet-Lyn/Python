...
# 使用 ffmpeg 压缩，类似命令：ffmpeg -i input.mkv -c:v libx264 -crf 18 -preset veryslow -c:a copy output.mkv。
...
# 支持的文件格式
video_formats = (".mkv", ".avi", ".f4v", ".flv", ".ts", ".mpeg", ".mpg", ".rm", ".rmvb", ".asf", ".wmv", ".mov", ".webm", ".mp4", ".ogv", ".ogm", ".ogg", ".vob")
...
def compress_and_remux_video(source_path, target_folder, relative_path):
    ...
    temp_output_path = os.path.join(target_subfolder, f"{file_name}_temp.mp4")  # H.264 输出为 mp4
    final_output_path = os.path.join(target_subfolder, f"{file_name}.mp4")
    
    # 构建 ffmpeg 命令（H.264 高质量压缩，音频不变）
    ffmpeg_command = [
    ...
    # 构建 mkvmerge 命令（仍用 mkvmerge 保持字幕，如果需要也可以改为 mp4 封装）
    mkvmerge_command = [
