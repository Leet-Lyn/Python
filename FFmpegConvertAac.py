...
def compress_audio(source_path, target_folder, relative_path):
    """
    压缩音频文件为AAC格式并保存到目标文件夹，保持目录结构。
    成功后删除源文件。
    ...
    # 目标AAC文件路径
    target_aac_path = os.path.join(target_subfolder, f"{file_name}.aac")
    ...
    try:
        ...
        # 检查目标文件是否存在且大小合理
        if os.path.exists(target_aac_path) and os.path.getsize(target_aac_path) > 0:
            print(f"音频压缩完成: {target_aac_path}")
...
for root, dirs, files in os.walk(source_folder):
    for file in files:
        if file.lower().endswith(audio_formats):  # 检查文件扩展名（忽略大小写）
            ...
            # 压缩音频
            compress_audio(source_path, target_folder, relative_path)
