...
MSG_MENU_CHOICE_PROMPT = "请输入选项 (0-7): "
...
MSG_MENU_6 = "6. 文件夹写入Excel并写入数据库（不比对删除）"
MSG_MENU_7 = "7. 整理数据库（排序去重）"
MSG_MENU_0 = "0. 退出"
...
# 选项 6 相关
MSG_OPT6_SORT_DB_START = "\n=== 整理SizeMD4数据库 ==="
MSG_OPT6_SORT_DB_NOT_FOUND = "数据库文件不存在: {}"
MSG_OPT6_SORT_DB_BACKUP_CREATED = "已创建备份: {}"
MSG_OPT6_SORT_DB_BACKUP_FAIL = "备份失败: {}"
MSG_OPT6_SORT_DB_COMPLETE = "整理完成。原始: {}，去重后: {}。"
MSG_OPT6_SORT_DB_PROCESS_ERROR = "整理过程中出错: {}"
MSG_OPT6_SORT_DB_RESTORED = "已从备份恢复原文件。"

# 选项 4 相关
MSG_OPT4_DEL_ED2K_START = "\n=== 多行ed2k链接从数据库删除 ==="
MSG_OPT4_DEL_ED2K_DELETED = "已删除: {}"
MSG_OPT4_DEL_ED2K_NOT_FOUND = "未找到: {}"
MSG_OPT4_DEL_ED2K_COMPLETE = "\n处理完成：删除 {} 条，未找到 {} 条。"

# 选项 5 相关
MSG_OPT5_DEL_MD4_START = "\n=== 多行SizeMD4值从数据库删除 ==="
MSG_OPT5_DEL_MD4_DELETED = "已删除: {}"
MSG_OPT5_DEL_MD4_NOT_FOUND = "未找到: {}"
MSG_OPT5_DEL_MD4_COMPLETE = "\n处理完成：删除 {} 条，未找到 {} 条。"
...
def process_folder_to_excel_and_db(
    ...
    check_duplicates=False 时跳过数据库比对，所有文件都处理且不删除。"""
    ...
    for idx, file_path in enumerate(all_files, 1):
        ...
        try:
            ...
            # 写入数据库（无论是否比对，新文件都要写入）
            if size_md4 not in size_md4_set:
...
def sort_and_dedup_size_md4_db(db_path: str) -> None:
    ...
    print(MSG_OPT6_SORT_DB_START)
    p = Path(db_path)
    if not p.is_file():
        print(MSG_OPT6_SORT_DB_NOT_FOUND.format(db_path))
        return
    ...
    try:
        ...
        print(MSG_OPT6_SORT_DB_BACKUP_CREATED.format(backup_path))
    except Exception as e:
        print(MSG_OPT6_SORT_DB_BACKUP_FAIL.format(e))
        return

    try:
        lines = [line.strip().upper() for line in p.read_text(encoding="utf-8").splitlines()
                 if line.strip()]
        original_count = len(lines)
        unique_sorted = sorted(set(lines))
        new_count = len(unique_sorted)
        p.write_text("\n".join(unique_sorted) + "\n", encoding="utf-8")
        print(MSG_OPT6_SORT_DB_COMPLETE.format(original_count, new_count))
    except Exception as e:
        print(MSG_OPT6_SORT_DB_PROCESS_ERROR.format(e))
        try:
            shutil.copy2(str(backup_path), str(p))
            print(MSG_OPT6_SORT_DB_RESTORED)
        except Exception:
            pass


def batch_delete_from_size_md4_database(db_path: str, size_md4_values: set) -> int:
    """从 SizeMD4 数据库文件中批量删除匹配的行，一次读写完成，返回删除数。"""
    p = Path(db_path)
    if not p.is_file():
        return 0
    lines = [line.strip() for line in p.read_text(encoding="utf-8").splitlines()
             if line.strip()]
    new_lines = [l for l in lines if l not in size_md4_values]
...
def delete_ed2k_links_from_db(size_md4_db_path: str) -> None:
    """从剪贴板读取多行 ed2k 链接，提取 SizeMD4 并批量从数据库删除。"""
    print(MSG_OPT4_DEL_ED2K_START)
    clipboard_text = get_clipboard_text()
    if not clipboard_text.strip():
        print(MSG_OPT2_CLIPBOARD_EMPTY)
        return
    ...
    # 批量删除
    deleted_count = batch_delete_from_size_md4_database(size_md4_db_path, to_delete)

    # 逐条报告结果
    for smd4 in to_delete:
        if smd4 in size_md4_set:
            print(MSG_OPT4_DEL_ED2K_DELETED.format(smd4))
        else:
            print(MSG_OPT4_DEL_ED2K_NOT_FOUND.format(smd4))

    not_found_count = len(to_delete) - deleted_count
    print(MSG_OPT4_DEL_ED2K_COMPLETE.format(deleted_count, not_found_count))


# ==================== 功能 6：多行 SizeMD4 值从数据库删除 ====================

def delete_size_md4_from_db(size_md4_db_path: str) -> None:
    """从剪贴板读取多行 SizeMD4 值并批量从数据库删除。"""
    print(MSG_OPT5_DEL_MD4_START)
    clipboard_text = get_clipboard_text()
    if not clipboard_text.strip():
        print(MSG_OPT2_CLIPBOARD_EMPTY)
        return
    ...
    for smd4 in to_delete:
        if smd4 in size_md4_set:
            print(MSG_OPT5_DEL_MD4_DELETED.format(smd4))
        else:
            print(MSG_OPT5_DEL_MD4_NOT_FOUND.format(smd4))

    not_found_count = len(to_delete) - deleted_count
    print(MSG_OPT5_DEL_MD4_COMPLETE.format(deleted_count, not_found_count))
