...
def safe_decode_filename(raw_bytes: bytes) -> str:
    """解码 torrent 中的文件名字段。先尝试 UTF-8，含乱码则回退 GBK。"""
    try:
        decoded = raw_bytes.decode("utf-8")
        if "�" in decoded:
            raise UnicodeDecodeError("utf-8", raw_bytes, 0, 0, "replacement chars")
        return decoded
...
def sanitize_filename(path_parts: list) -> str:
    """路径拼接，分隔符替换为下划线。"""
    return "_".join(str(p).replace("/", "_").replace("\\", "_") for p in path_parts)


def copy_to_clipboard(text: str) -> None:
    """文本 → Windows 剪贴板（clip.exe）。"""
    try:
        subprocess.run(
            ["clip.exe"],
            input=text,
...
def is_file_downloaded(download_dir: Path, path_parts: list) -> bool:
    """检查文件是否已下载成功（无 .bc! 后缀）。
    path_parts 是 torrent 中的原始路径分量列表 [b"dir", b"file.ext"]。
    返回 True 表示已下载成功，False 表示未下载或仅存在 .bc!。"""
    if not path_parts:
        return False

    decoded = [safe_decode_filename(p) for p in path_parts]
    full_path = download_dir / Path(*decoded)
...
def collect_undownloaded(tree_node, download_dir: Path):
    ...
    for f_entry in tree_node.get("_files", []):
        ...
        if not is_file_downloaded(download_dir, raw_parts):
...
def count_status_in_subtree(tree_node, download_dir: Path) -> tuple:
    ...
    for f_entry in tree_node.get("_files", []):
        raw_parts = f_entry.get(b"path", [])
        total += 1
        if is_file_downloaded(download_dir, raw_parts):
            downloaded += 1
