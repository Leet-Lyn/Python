...
def get_operation_parameters():
    ...
    # 修改点：不再减1，返回原始输入整数（支持负数）
    try:
        raw_index = int(input("第几个单元（从 1 开始，负数表示倒数）: "))
    except ValueError:
        print("输入无效，请重试。")
    ...
    if choice == "1":
        ...
        if move_type in ("向前移动", "向后移动"):
            try:
                ...
                print("输入无效，默认为0。")
                count = 0
        return "移动", raw_index, move_type, count

    if choice == "2":
        return "删除", raw_index, None, None
    ...
    if choice == "3":
        try:
            ...
            print("输入无效，默认为最后。")
            target = -1
        return "复制", raw_index, target, None
...
def process_text(text):
    ...
    # 逐行处理单元操作
    new_lines = []
    for idx, line in enumerate(lines, start=1):  # 记录行号用于提示
        ...
        # 根据用户输入的原始索引 p1 计算实际 0 基索引
        if p1 > 0:
            actual_index = p1 - 1
        elif p1 < 0:
            actual_index = n + p1   # 例如 n=5, p1=-1 → 5+(-1)=4
        else:  # p1 == 0，视为无效输入
            actual_index = -1

        # 检查索引是否有效
        if actual_index < 0 or actual_index >= n:
            print(f"第 {idx} 行：索引 {p1} 无效（该行共有 {n} 个单元），跳过此行。")
            new_lines.append(line)
            continue

        # 执行对应的单元操作
        if op == "移动":
            units = move_unit(units, actual_index, p2, p3)
        elif op == "删除":
            units = delete_unit(units, actual_index)
        elif op == "复制":
            units = copy_unit(units, actual_index, p2)
        else:
            new_lines.append(line)
            continue
