...
# 在脚本开始前询问我源文件位置。
# 1. 如果源文件格式为 bmp、jpg、jpeg、png 或静态 webp 格式，则使用 magick 压缩成 jpg 格式，使用类似命令：magick convert input.png -quality 75 output.jpg。
# 2. 如果图片文件格式为 gif 或动态 webp 格式或 mp4 格式，则使用 magick 压缩成 gif 格式，类似命令：magick convert input.webp -fuzz 5% -quality 75 -layers Optimize output.gif。
# 生成的文件替换源文件。
# 如此循环，再次前询问我源文件位置。
...
def compress_file():
    ...
    while True:
        ...
        try:
            if file_ext in ['.bmp', '.jpg', '.jpeg', '.png']:  # 静态图片格式
