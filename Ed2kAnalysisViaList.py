...
# 在脚本开始前询问我源文件位置，默认为“e:\Documents\Creations\Scripts\Attachments\Python\Ed2kList.txt”（按回车表示默认，按“c”读取剪贴板，并写入“e:\Documents\Creations\Scripts\Attachments\Python\Ed2kList.txt”）。
# 该文件（或剪贴板）内包含许多 ed2k 链接，每行一个，顺序读取每个 ed2k 链接。
# 每个链接都是百分号编码(Percent-encoding)，请将其转回原来链接。在源文件位置下生成新的文件（读取剪贴板则在 “e:\Documents\Creations\Scripts\Attachments\Python\”下），文件名为“Ed2kList.new.txt”。
...
def main():
    # 询问源文件位置，默认值为"e:\Documents\Creations\Scripts\Attachments\Python\Ed2kList.txt"
    user_input = input("请输入源文件位置（默认为'e:\\Documents\\Creations\\Scripts\\Attachments\\Python\\Ed2kList.txt'，按回车使用默认，按'C'或'c'读取剪贴板）：").strip()
    
    if user_input.lower() == 'c':
        # 用户选择从剪贴板读取
        source_file_path = "e:\\Documents\\Creations\\Scripts\\Attachments\\Python\\Ed2kList.txt"
        source_folder = "e:\\Documents\\Creations\\Scripts\\Attachments\\Python\\"
